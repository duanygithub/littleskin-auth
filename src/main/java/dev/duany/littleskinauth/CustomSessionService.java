package dev.duany.littleskinauth;
...
public class CustomSessionService implements MinecraftSessionService {
    private static final Logger LOGGER = LoggerFactory.getLogger(YggdrasilMinecraftSessionService.class);
    private final MinecraftClient client;
    private final String baseUrl;
    private final URL joinUrl;
    private final URL checkUrl;
    private final String lsBaseUrl;
    private final URL lsJoinUrl;
    private final URL lsCheckUrl;
    ...
    public CustomSessionService(final Proxy proxy, final Environment env) {
        ...
        lsBaseUrl = "https://littleskin.cn/api/yggdrasil/sessionserver" + "/session/minecraft/";
        lsJoinUrl = HttpAuthenticationService.constantURL(lsBaseUrl + "join");
        lsCheckUrl = HttpAuthenticationService.constantURL(lsBaseUrl + "hasJoined");
    }
    ...
    public void joinServer(final UUID profileId, final String authenticationToken, final String serverId) throws AuthenticationException {
        ...
        try {
            ...
            try {
                client.post(lsJoinUrl, request, Void.class);
    ...
    public ProfileResult hasJoinedServer(final String profileName, final String serverId, @Nullable final InetAddress address) throws AuthenticationUnavailableException {
        ...
        final URL lsUrl = HttpAuthenticationService.concatenateURL(lsCheckUrl, HttpAuthenticationService.buildQuery(arguments));
        try {
            HasJoinedMinecraftServerResponse response = client.get(url, HasJoinedMinecraftServerResponse.class);
            if(response == null || response.id() == null) {
                response = client.get(lsUrl, HasJoinedMinecraftServerResponse.class);
            }
            if (response != null && response.id() != null) {
    ...
    private ProfileResult fetchProfileUncached(final UUID profileId, final boolean requireSecure) {
        try {
            ...
            URL lsUrl = HttpAuthenticationService.constantURL(lsBaseUrl + "profile/" + UndashedUuid.toString(profileId));
            lsUrl = HttpAuthenticationService.concatenateURL(lsUrl, "unsigned=" + !requireSecure);

            MinecraftProfilePropertiesResponse response = client.get(url, MinecraftProfilePropertiesResponse.class);
            if (response == null) {
                response = client.get(lsUrl, MinecraftProfilePropertiesResponse.class);
                if(response == null) {
